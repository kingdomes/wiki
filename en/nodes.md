---
title: Node Info
description: 
published: true
date: 2026-07-12T08:29:31.551Z
tags: 
editor: markdown
dateCreated: 2026-07-12T08:29:31.551Z
---

# Node Information

This page lists all nodes currently operated by AFNOOI-MNT in the DN42 network ~~?~~.

---

## Node Overview

| Node Code | Location | Bandwidth | Status | IPv4 | IPv6 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **CN-1** | Hengyang, China | 500 Mbps | Online | ✔️🌧️ | ✔️ |
| **CN-2** | Hengyang, China | 500 Mbps | Online | ✔️🌧️ | ✔️ |
| **CN-3** | Hengyang, China | 500 Mbps | Online | ✔️🌧️ | ✔️ |
| **US-1** | Utah, USA | 1 Gbps | Online | ✔️🌧️ | ✔️ |
| **US-2** | Los Angeles, USA | 100 Mbps | Online | ✔️ | ✔️🌧️ |
| **HK-1** | Hong Kong, China | 100 Mbps | Online | ✔️🌧️ | ✔️ |
| **TW-1** | Taiwan, China | 100 Mbps | Online | ✔️🌧️ | ✔️ |
| **MD-1** | Chișinău, Moldova | 100 Mbps | Online | ✔️🌧️ | ✔️ |
| **RU-1** | Moscow, Russia | 1 Gbps | Online | ✔️🌧️ | ✔️ |

**Ps: 🌧️ Behind NAT**

---

## CN-1 (China)

- **Location**: Hengyang, China
- **Bandwidth**: 500 Mbps
- **Hostname**: `cn1.115411.xyz`
- **IPv4 Address**: `172.23.57.132`
- **IPv6 Address**: `fdf1:8356:996d:7e8f:90a1:b2c3:d4e5:f607`
- **Link-Local Address (LLA)**: `fe80::2e0:4cff:fe68:42`
- **WireGuard Public Key**: `Yjtn/QiknbveBgJIibn4jE3Zj54Tu6sOod0dwCvQZQo=`

---

## CN-2 (China)

- **Location**: Hengyang, China
- **Bandwidth**: 500 Mbps
- **Hostname**: `cn2.115411.xyz`
- **IPv4 Address**: `172.23.57.131`
- **IPv6 Address**: `fdf1:8356:996d:b2c4:d5e6:f708:192a:3b4c`
- **Link-Local Address (LLA)**: `fe80::d764:d302:ab24:f81c`
- **WireGuard Public Key**: `mktoQoBIECGUJNQIMTW05VhhVvJAct3N6xO4n1OEy0M=`

---

## CN-3 (China)

- **Location**: Hengyang, China
- **Bandwidth**: 500 Mbps
- **Hostname**: `cn3.115411.xyz`
- **IPv4 Address**: `172.23.57.130`
- **IPv6 Address**: `fdf1:8356:996d:1a3b:2c4d:5e6f:7081:92a3`
- **Link-Local Address (LLA)**: `fe80::2e0:4cff:fe68:39`
- **WireGuard Public Key**: `aBhlO79WRv08G2ypQPnibcd8WiUeZr7/9YVAx+tSeDU=`

---

## US-1 (United States)

- **Location**: Utah, USA
- **Bandwidth**: 1 Gbps
- **Hostname**: `us1.115411.xyz`
- **IPv4 Address**: `172.23.57.148`
- **IPv6 Address**: `fdf1:8356:996d::1448`
- **Link-Local Address (LLA)**: `fe80::3443`
- **WireGuard Public Key**: `JtGlFP42nSNKRL6rUu8IcrGxYZZeDfrYSHI1yt4G0CY=`

---

## US-2 (United States)

- **Location**: Los Angeles, USA
- **Bandwidth**: 100 Mbps
- **Hostname**: `us2.115411.xyz`
- **IPv4 Address**: `172.23.57.133`
- **IPv6 Address**: `fdf1:8356:996d:33::`
- **Link-Local Address (LLA)**: `fe80::1266:6aff:fe58:6770`
- **WireGuard Public Key**: `JtGlFP42nSNKRL6rUu8IcrGxYZZeDfrYSHI1yt4G0CY=`

---

## HK-1 (Hong Kong, China)

- **Location**: Hong Kong, China
- **Bandwidth**: 100 Mbps
- **Hostname**: `hk1.115411.xyz`
- **IPv4 Address**: `172.23.57.156`
- **IPv6 Address**: `fdf1:8356:996d:53::1`
- **Link-Local Address (LLA)**: `fe80::216:3eff:fe69:881b`
- **WireGuard Public Key**: `IUOW25XPxZO2T0Yv4HUdBVZrLveNMJEKu5Qvg4M2Xg8=`

---

## TW-1 (Taiwan, China)

- **Location**: Taiwan, China
- **Bandwidth**: 100 Mbps
- **Hostname**: `tw1.115411.xyz`
- **IPv4 Address**: `172.23.57.158`
- **IPv6 Address**: `fdf1:8356:996d:7c3a:91b2:4e8f:d6c1:20f5`
- **Link-Local Address (LLA)**: `fe80::1266:6aff:fe0d:2c64`
- **WireGuard Public Key**: `57uCMqoy3NjjNjBspB/qzdp8dY5Knkdwe+I/tEIz5wo=`

---

## MD-1 (Moldova)

- **Location**: Chișinău, Moldova
- **Bandwidth**: 100 Mbps
- **Hostname**: `md1.115411.xyz`
- **IPv4 Address**: `172.23.57.157`
- **IPv6 Address**: `fdf1:8356:996d::1445`
- **Link-Local Address (LLA)**: `fe80::5067:45ff:fe12:38de`
- **WireGuard Public Key**: `PDl9G+pPI3nOREiP4u3NCRJ+m/pJb8LU9LQ+3MwAakc=`

---

## RU-1 (Russia)

- **Location**: Moscow, Russia
- **Bandwidth**: 1 Gbps
- **Hostname**: `ru1.115411.xyz`
- **IPv4 Address**: `172.23.57.125`
- **IPv6 Address**: `fdf1:8356:996d:54::1`
- **Link-Local Address (LLA)**: `fe80::18af:deff:fe49:74ff`
- **WireGuard Public Key**: `3CrWyAX10kRqNWXh7PqjPcWYksq/OT0Pd2hOO8uyHAE=`

---

## Notes

- **LLA (Link-Local Address)**: Used for BGP peering or tunnel setup, typically within the `fe80::/10` range ~~?~~.
- **WG Public Key**: Used for WireGuard tunnel authentication. Please keep it secure and only share with trusted peers.~~

For peering requests, please refer to the [Peering page](./Peering-with-Me) for detailed instructions.